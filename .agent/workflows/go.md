---
description: Запуск цикла разработки по AI-контракту — lint, test, исправление до победного.
---
...
3. **Реализуй** код (React 19, TypeScript strict, function components).
4. **Цикл разработки** (повторяй сначала в конце каждого логического блока работ и после исправления ошибок в командах ниже):
   - Запусти `pnpm lint` → исправь ошибки если есть.
   - Запусти `pnpm test` → исправь если красное.
5. **Финальная проверка** (один раз в конце задачи):
   - Запусти `pnpm typecheck` → исправь если красное.
   - Запусти `pnpm test:coverage` → исправь если красное. Также доведи coverage до 100% измененных файлов в соответствии с контрактом (не тестировать фреймворк, тривиальные обёртки и т.д.).
6. **Только если всё зеленое** → готово к сдаче.
7. **Самоконтроль**: перепроверь критические запреты (особенно комментарии, inline-стили, `any`).
